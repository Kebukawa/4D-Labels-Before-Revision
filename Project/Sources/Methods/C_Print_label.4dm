	...
	: ($File_path="form:@")
		...
		If (Bool:C1537(Get database parameter:C643(Is host database a project:K37:99)))
			...
			If (Is compiled mode:C492(*))
				...
				$file:=Folder:C1567(fk database folder:K87:14; *).files().query("extension = :1"; ".4DZ").pop()
				
				If (Bool:C1537($file.exists))
					
					$file:=ZIP Read archive:C1637($file).root.file("Project/Sources/TableForms/"+String:C10($Lon_table)+"/"+$Txt_form+"/form.4DForm")
					
				End if 
				
			Else 
				
				$file:=Folder:C1567(fk database folder:K87:14; *).file("Project/Sources/TableForms/"+String:C10($Lon_table)+"/"+$Txt_form+"/form.4DForm")
		...
		Else 
			...
			Else 
				...
				$o:=FORM Convert to dynamic:C1570($Ptr_table->; $Txt_form)
		...
		If ($o#Null:C1517)
			...
			$Lon_height:=Num:C11($o.markerBody)-Num:C11($o.markerHeader)  // MarkerHeader is undefined if equal to 0
